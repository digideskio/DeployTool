require_relative "deploy/#{ fetch(:stage) }"
...
# Sphinx
set :sphinx_conf_file, "#{ fetch :configs_dir }/sphinx.conf"
...
set :redis_dir,  "#{ shared_path }/db/REDIS"
redis_file_name = "redis_#{ fetch :redis_port }_#{ fetch :redis_ns }"
set :redis_db_file, redis_file_name

set :redis_init_file,   "_REDIS.rb"
set :redis_config_file, "#{ redis_file_name }.conf"
...
# SIDEKIQ params
set :sidekiq_init_file,   "_SIDEKIQ.rb"
set :sidekiq_pid,         "sidekiq.pid"
set :sidekiq_config_file, "sidekiq.yml"
...
# NGINX/SSL
if Configs.nginx.ssl
  set :ssl,           true
  set :ssl_dir,       :SSL_certs
  set :ssl_type,      Configs.ssl.type

  set :ssl_cert_name, Configs.ssl.cert_name
  set :ssl_pkey_name, Configs.ssl.pkey_name
  set :ssl_bundled_crt_name, 'ssl-bundle.crt'
end
...
linked_files_addons |= %w[ config/secrets.yml   ]
linked_files_addons |= %w[ config/app_config.rb ]                        if theme?
linked_files_addons |= %w[ config/newrelic.yml  ]                        if new_relic?
linked_files_addons |= %w[ config/schedule.rb   ]                        if whenever?

if sidekiq?
  linked_files_addons |= %W[
    config/initializers/#{ fetch :sidekiq_init_file }
    config/#{ fetch :sidekiq_config_file }
  ]
end

if rails_config?
  linked_files_addons |= %W[ config/settings/#{ fetch(:stage) }.yml ]
end
...
if sphinx?
  ...
  linked_files_addons |= %w[ config/sphinx.conf ]
end

if redis?
  linked_files_addons |= %W[
    config/initializers/#{ fetch :redis_init_file }
    config/#{ fetch :redis_config_file }
  ]
end

set :linked_files, %w[ config/database.yml ] | linked_files_addons
set :linked_dirs,  %w[ bin log search_index tmp db/REDIS vendor/bundle public/system public/uploads ]
